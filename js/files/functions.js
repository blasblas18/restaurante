...
function cargando(capa,img) { 
    if(img==1)   imgLoad = "6";    if(img==2)   imgLoad = "7";    if(img==3)   imgLoad = "6s";    
    $(capa).html('<br /><center><img src="img/elements/loaders/'+imgLoad+'.gif"/></center>');
}
function loading(name,overlay) {  
    if(name=='0')   name='Cargando';
...
{   $.jGrowl( 'Ud. no puede ver esta seccion',
    {   theme: 'growl-warning', 
        header: 'Sin permisos!' 
    });
}

function mostrar_html(capa, resp)
{   if(resp == 'SinPermiso')        sin_permiso();
    else            $(capa).html(resp);    
}
...
$(function() {
	...
	$('.showmenu').click(function () {
		$('.actions-wrapper').slideToggle(1000);
	});
